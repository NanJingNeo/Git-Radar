...
%peaks代表这段数据上的目标的轨迹
%解决方法1：
%进行两次的vncmd测心跳（两个线程），每次输入的是不同的peaks和tops
tic;
tops = tops';
peaks = peaks';
...
[~,f] = STFT(Sig',SampFreq,Nfrebin,window);
...
M=2;
delta = 10;
[fidexmult, ~] = extridge_mult(Sig, SampFreq, num, delta, beta1,bw,Nfrebin,window);
...
[IFmset, IA, smset, ~] = VNCMD2_2(Sig,SampFreq,iniIF,alpha,beta,var,tol);
